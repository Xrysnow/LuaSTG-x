...
std::wstring lstg::Utf8ToUtf16(const char* p)
{
	wstring tRet;
	tRet.reserve(128);
...
std::string lstg::Utf16ToUtf8(const wchar_t* p)
{
	string tRet;
	tRet.reserve(128);
...
void lstg::Utf8ToUtf16(const char* p, std::wstring& ret)
{
	ret.clear();
	Convert<Utf8Decoder, Utf16Encoder, wchar_t>(p, ret);
}

void lstg::Utf16ToUtf8(const wchar_t* p, std::string& ret)
{
	ret.clear();
