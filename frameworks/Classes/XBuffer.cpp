...
std::streamsize BufferStreamBuf::xsgetn(char* s, std::streamsize count)
{
	...
	const auto avail = (std::streamsize)buf->size() - g_off;
	if (avail <= 0)
		return 0;
	count = std::min(count, avail);
	std::memcpy(s, buf->data() + g_off, count);
	gupdate(g_off + count);
	return count;
}
...
std::streamsize BufferStreamBuf::xsputn(const char* s, std::streamsize count)
{
	if (!buf || !buf->isWritable())
		return 0;
	const auto p_off = poff();
	if (p_off + count > buf->size())
	{
		if (buf->isResizable())
		{
			buf->resize(p_off + count);
			gupdate(goff());
		}
		else
		{
			count = std::max((std::streamsize)buf->size() - p_off, (std::streamsize)0);
		}
	}
	std::memcpy(buf->data() + p_off, s, count);
	pupdate(p_off + count);
	return count;
}
...
void BufferStreamBuf::pupdate(std::streamsize offset)
{
	if (!buf)
		return;
	const auto base = (char*)buf->data();
	setp(base + offset, base + buf->size());
}
...
void BufferStreamBuf::gupdate(std::streamsize offset)
{
	if (!buf)
		return;
	const auto base = (char*)buf->data();
	setg(base, base + offset, base + buf->size());
}
